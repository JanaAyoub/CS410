...
int main()
{
    ...
    if (newfile.is_open())
    {
        ...
        if(tp=="ALPHABET")
        {
            ...
            {
                getline(newfile, tp);
                for(int t=0;tp[t];t=t+2){

                    int i=indexof(string(1,tp[t]));
                    for(j=1;j<=sym;j++)
                    {
                        if(Transition[0][j]==string(1,tp[t+2]))
                        {
                            Transition[i][j]=Transition[i][j] + string(1,tp[t+4]);
